...
int AVOID = 1;
int exist[5] =  {8, 10, 2, 6, 3};  // resources existing in the system
...
void *threadfunc1(void* a){
    ...
    setarray(claim, NUMR, 5, 7, 2, 6, 2);
    rm_claim(claim);

    setarray(request1, NUMR, 3, 6, 1, 4, 2);
    pr (tid, "REQ", NUMR, request1);
    rm_request(request1);

    sleep(2);

    setarray(request2, NUMR, 1, 1, 1, 1, 0);
    pr (tid, "REQ", NUMR, request2);
    rm_request(request2);

    sleep(2);

    setarray(request3, NUMR, 1, 0, 0, 1, 0);
...
void *threadfunc2(void* a){
    ...
    setarray(claim, NUMR, 2, 3, 1, 4, 3);
    rm_claim(claim);

    setarray(request1, NUMR, 1, 2, 1, 1, 2);
    pr (tid, "REQ", NUMR, request1);
    rm_request(request1);

    sleep(2);

    setarray(request2, NUMR, 0, 1, 0, 3, 1);
    pr (tid, "REQ", NUMR, request2);
    rm_request(request2);

    sleep(2);

    setarray(request3, NUMR, 1, 0, 0, 0, 0);
    pr (tid, "REQ", NUMR, request3);
    rm_request(request3);
...
void *threadfunc3(void* a){
    ...
    setarray(claim, NUMR, 8, 9, 0, 6, 2);
    rm_claim(claim);

    setarray(request1, NUMR, 3, 7, 0, 4, 0);
    pr (tid, "REQ", NUMR, request1);
    rm_request(request1);

    sleep(2);

    setarray(request2, NUMR, 1, 1, 0, 1, 2);
    pr (tid, "REQ", NUMR, request2);
    rm_request(request2);

    sleep(2);

    setarray(request3, NUMR, 4, 1, 0, 1, 0);
    pr (tid, "REQ", NUMR, request3);
    rm_request(request3);
...
int main (int argc, char **argv){
    ...
    rm_print_state("state after join");
