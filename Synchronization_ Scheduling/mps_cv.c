...
void *schedule(void *args) {
    ...
    if (strcmp(targs->alg, RR_STRING) == 0){
        schedule_rr_cv(&conds[index], targs->i, outmode, start_time, &finished_lock, &finished_list, &mutex[index], &ready_queues_list[index], targs->q);
    } else if (strcmp(targs->alg, FCFS_STRING) == 0){
        schedule_fcfs_cv(&conds[index], targs->i, outmode, start_time, &finished_lock, &finished_list, &mutex[index], &ready_queues_list[index]);
    }else if (strcmp(targs->alg, SJF_STRING) == 0) {
        schedule_sjf_cv(&conds[index], targs->i, outmode, start_time, &finished_lock, &finished_list, &mutex[index], &ready_queues_list[index]);
    }else {
...
int main(int argc, char *argv[]) {
   ...
   int opt;
    ...
    conds = (pthread_cond_t*) malloc(temp_int * sizeof(pthread_cond_t));
            ...
            if (sscanf(line, "PL %d", &pl) == 1) {
                ...
                else
                {
                    ...
                    if (strcmp(qs, "RM") == 0)
                    {
                        ...
                        pthread_cond_signal(&conds[(process_count-1)%n]);
                    ...
                    else if(strcmp(qs, "LM") == 0)
                    {
                        ...
                        pthread_cond_signal(&conds[current_min_index]);
